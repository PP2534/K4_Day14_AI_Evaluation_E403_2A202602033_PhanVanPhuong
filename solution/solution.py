...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for rank, chunk in enumerate(contexts, start=1):
            ...
            if is_relevant:
                num_relevant += 1
                total += num_relevant / rank

        if num_relevant == 0:
            return 0.0
        score = total / num_relevant
        return max(0.0, min(1.0, score))
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an evaluation judge. Score the answer against each rubric "
            "criterion from 0.0 to 1.0.\n"
            f"Question: {question}\n"
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_lines}\n"
            'Reply with JSON only, e.g. {"accuracy": 0.8}.'
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(scores_batch) > 1:
            first_scores = list((scores_batch[0].get("scores") or {}).values())
            rest_scores = [
                score
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        def _avg(attr: str, ignore_none: bool = False) -> float | None:
        ...
        return {
            ...
            "avg_context_recall": _avg("context_recall", ignore_none=True),
            "avg_context_precision": _avg("context_precision", ignore_none=True),
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        for name, new_val, baseline_val in (
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        lowest_keys = [name for name, value in scores.items() if value == lowest]

        if len(lowest_keys) > 1:
            return "Multiple issues detected — review full pipeline"
        if lowest_keys[0] == "faithfulness":
            return "Context is missing or irrelevant — improve retrieval"
        if lowest_keys[0] == "relevance":
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = (
                suggestions[(index - 1) % len(suggestions)]
                if suggestions
                else "Review and retest"
            )
            lines.append(
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append(
                "Implement a hallucination checker / grounding guardrail that verifies "
                "every claim in the answer against the retrieved context before responding."
            )
        if categories.get("irrelevant"):
            suggestions.append(
                "Sharpen the system prompt and intent routing so the agent stays on-topic "
                "and directly answers the user's question."
            )
        if categories.get("incomplete"):
            suggestions.append(
                "Increase the context window / top-k and instruct the agent to list all "
                "steps and conditions from the policy before answering."
            )
        if categories.get("off_topic"):
            suggestions.append(
                "Improve intent detection and add an out-of-scope fallback that politely "
                "declines or asks for clarification."
            )
        if categories.get("refusal"):
            suggestions.append(
                "Relax overly strict guardrails so the agent answers within-scope questions confidently."
            )
        if not suggestions and failures:
            suggestions.append(
                "Review retrieval quality and golden dataset coverage before retesting."
            )
        return suggestions
